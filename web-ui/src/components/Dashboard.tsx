...
const Dashboard: React.FC = () => {
    ...
    const [budgetGoals] = useState<any[]>(() => {
    ...
    // Prepare expense trend chart data (last 7 days)
    ...
    const expenseChartData = last7Days.map(date => {
        const dayExpenses = expenses.filter(exp => exp.createdDate.split('T')[0] === date);
    ...
    // Check for upcoming subscription payments (next 7 days)
    ...
    const stats = [
        ...
        },
        {
            ...
            change: overdueDebts.length > 0 ? `${overdueDebts.length} overdue` : 'On track',
            changeColor: overdueDebts.length > 0 ? 'text-red-600' : 'text-green-600'
        },
        {
            title: 'Investments',
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div className="space-y-4">
                            ...
                            <div className="flex items-start space-x-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-red-700">
                                        You have {overdueDebts.length} overdue debt{overdueDebts.length > 1 ? 's' : ''} totaling
                                        ${overdueDebts.reduce((sum, debt) => sum + debt.amount, 0).toFixed(2)}
                                    </p>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="flex items-start space-x-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-yellow-700">
                                        {upcomingPayments.length} subscription payment{upcomingPayments.length > 1 ? 's' : ''} due in the next 7 days
                                    </p>
                                </div>
                            </div>
                        )}
                        ...
                        {/* No Alerts */}
                        {overdueDebts.length === 0 && upcomingPayments.length === 0 && (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h2 className="text-lg font-semibold text-gray-900 mb-4">Budget Goals Progress</h2>
                <div className="space-y-4">
                    {currentMonthGoals.length > 0 ? (
                        currentMonthGoals.map((goal: any) => {
                            ...
                            const progressColor = percentage >= 100 ? 'bg-red-500' 
